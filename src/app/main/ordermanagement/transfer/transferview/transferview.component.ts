...
export class TransferviewComponent implements OnInit {
    ...
    constructor(public sessionManageMent: SessionManageMent,
        ...
        this.TransferOrderNo = this.route.snapshot.paramMap.get('response')
    }
    ...
    get_sale_order_info() {
        try {
            this.spinner.show();
            const json = {
              TransferNo: this.TransferOrderNo,
            ...
            this.webApiHttp.Post(this.webApiHttp.ApiURLArray.TransferOrderInfo, json).then(result => {
                ...
                } else if (result.transferOrder[0].condition == 'True') {
                    this.order = result
                    this.dataSource = new MatTableDataSource<any>(result.transferOrder);
